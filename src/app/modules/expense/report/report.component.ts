...
export class ReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reportForm
      .get('report')
      ?.valueChanges.subscribe((value) => this.toggleDates(value));
  }
  ...
  getReport(formdata: any): void {
    if (this.reportForm.valid) {
      if (
        this.reportForm.value.report === 'opt2' &&
        new Date(this.reportForm.value.startdt) >
          new Date(this.reportForm.value.enddt)
      ) {
  ...
  fetchReport(userid: string, formval: any) {
    this.expenseService.getExpenses(userid, formval).subscribe((data: any) => {
      if (data.success === false) {
        ...
        if (formval.report === 'opt1') {
          this.reportTitle =
            'for ' + this.datePipe.transform(new Date(), 'MMM y');
        } else if (formval.report === 'opt2') {
          this.reportTitle =
            'between ' +
            this.datePipe.transform(new Date(formval.startdt), 'd MMM y');
          this.reportTitle +=
            ' and ' +
  ...
  getExpTot(userid: string, formval: any) {
    this.expenseService
      .getExpenseTotal(userid, formval)
      .subscribe((res: any) => {
  ...
  confirmDel(idx: number, expid: string) {
    if (confirm('Do you really want to delete this record?')) {
      this.expenseService.delExpense(expid).subscribe((data: any) => {
        if (data.success === false) {
          if (data.errcode) {
            this.authService.logout();
            this.router.navigate(['login']);
